...
check_command() {
    ...
    local compose_cmd="${2:-}" # Optional docker compose command
    ...
    if [ $exit_code -ne 0 ]; then
        ...
        if [[ -n "$compose_cmd" ]]; then
             if command -v ${compose_cmd%% *} &> /dev/null && $compose_cmd ps -q frappe &> /dev/null; then
                 error "Attempting to show last 50 lines of Frappe logs:" # Goes to stderr
                 $compose_cmd logs --tail=50 frappe || true # Ignore error if logs fail
             fi
        fi
        exit $exit_code
    fi
